...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// -------------------------------------------------------------- locales
		const zh = {
			"section.label": "隐藏的工作区",
			"section.title": "隐藏的工作区",
			"section.desc": "隐藏一个工作区，它会连同名下的会话一起从侧边栏消失。插件会把该工作区里尚未归档的会话自动归档，所以它们不会掉进侧边栏的「未分组」。取消隐藏时，只恢复这些由隐藏动作归档的会话；隐藏之前就已经归档的会话保持归档不动。文件夹和会话记录始终留在磁盘上。",
			"section.empty": "当前没有任何工作区。",
			"section.count": "已隐藏 {n} 个工作区",
			"section.countZero": "当前没有隐藏任何工作区",
			...
			"switch.toHide": "在侧边栏隐藏「{name}」",
			"switch.toShow": "在侧边栏恢复显示「{name}」",
		...
		const en = {
			"section.label": "Hidden workspaces",
			"section.title": "Hidden workspaces",
			"section.desc": "Hiding a workspace removes it and its conversations from the sidebar. Any session in it that is not archived yet is archived automatically, so nothing spills into the sidebar's \"Ungrouped\" bucket. Unhiding restores exactly the sessions the hide archived; sessions that were already archived before are left archived. Folders and session logs always stay on disk.",
			"section.empty": "There are no workspaces yet.",
			"section.count": "{n} hidden workspace(s)",
			"section.countZero": "Nothing is hidden right now",
			...
			"switch.toHide": "Hide \"{name}\" from the sidebar",
			"switch.toShow": "Show \"{name}\" in the sidebar again",
		...
		function WorkspaceRow({ item, hidden, archivedCount, t, onToggle }) {
			...
			return h("div", { style: styles.row, key: id },
				...
				h(Switch, {
					checked: hidden === true,
					label: hidden ? t("switch.toShow", { name: label }) : t("switch.toHide", { name: label }),
					title: hidden ? t("switch.toShow", { name: label }) : t("switch.toHide", { name: label }),
		...
		function WorkspaceHideSection(props) {
			...
			const toolbar = h("div", { style: styles.toolbar, key: "toolbar" },
				h("div", { style: styles.count }, total === 0 ? t("section.countZero") : t("section.count", { n: total })),
				h(Button, { size: "sm", disabled: total === 0 && pending.size === 0, onClick: () => restoreAll() }, t("action.restoreAll"))
			);
			...
			const displayOnly = total > 0 && manage?.canRestore !== true
