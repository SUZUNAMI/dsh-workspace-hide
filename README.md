...
在 DSH 侧边栏**隐藏指定工作区**，连同它名下的会话一起，并在「设置 → 隐藏的工作区」里随时恢复。

纯本地、可逆、不删数据：工作区文件夹和会话记录始终留在磁盘上；隐藏时会话会被**自动归档**
（而不是掉进「未分组」），恢复时只把这一批归档撤销回来。
...
（官方 `ui-workspace` 的 `groupByWorkspace()` 只统计**可见**工作区名下的会话，
剩下的全部当成 stray 归入 `group.ungrouped`）。
